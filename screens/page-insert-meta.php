...
<a href="admin.php?page=<?php echo $reusables->directory;?>/insert&f=meta&region_id=<?php echo $_GET["region_id"];?>&paged=<?php echo $_REQUEST["paged"]-1;?>&n=<?php echo $numberposts?>&n=<?php echo $numberposts?>" class="previous page-numbers">&laquo;</a>
<?php } ?>
...
<?php } else { ?>
<a href="admin.php?page=<?php echo $reusables->directory;?>/insert&f=meta&region_id=<?php echo $_GET["region_id"];?>&paged=<?php echo $i;?>&n=<?php echo $numberposts?>" class="page-numbers"><?php echo $i?></a>
<?php } ?>

<?php } ?>

<?php if($_REQUEST["paged"] != $pages){ ?>
<a href="admin.php?page=<?php echo $reusables->directory;?>/insert&f=meta&region_id=<?php echo $_GET["region_id"];?>&paged=<?php echo $_REQUEST["paged"]+1;?>&n=<?php echo $numberposts?>" class="next page-numbers">&raquo;</a>
<?php } ?>
<?php } ?>
...
foreach($reuss as $reus){
   ...
   <div class="row-actions"><a href="javascript://" onclick="if(meta = set_meta(['.(!empty($metas)?implode(",",$metas):null).'])){ opener.add_reus('.$_GET["region_id"].','.$reus->ID.',\''.$reus->post_title.'\',meta);window.close(); }">Insert</a></div></td></tr>';
